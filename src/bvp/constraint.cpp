...
VectorXd ConstraintWithSystem::operator()(const VectorXd& x) const
{
    ...
    */
    ...
    std::cout << "absolution value of x(duration_start): " << std::abs(x(duration_start)) << std::endl;
    errs(0) = start_dynamics(x.segment(0,state_dim),
                             x.segment(control_start, control_dim),
                             std::abs(x(duration_start)),
                             x.segment(state_dim,state_dim));
    errs(_n_steps+1) = time_min_constraint(std::abs(x(duration_start)));
    ...
    for (unsigned i=1; i < _n_steps-2; i+=1)
    {
      ...
      errs(i) = dynamic_constraint(x.segment(i*state_dim,state_dim),
                                   ...
                                   std::abs(x(duration_start+i)),
                                   x.segment((i+1)*state_dim,state_dim));
      std::cout << "abolution value of x(duration_start+i): " << std::abs(x(duration_start+i)) << std::endl;
      ...
      errs(_n_steps+1+i) = time_min_constraint(std::abs(x(duration_start+i)));
    ...
    errs(_n_steps-2) = term_dynamics(x.segment((_n_steps-2)*state_dim,state_dim),
                                      ...
                                      std::abs(x(duration_start+(_n_steps-2))),
                                      x.segment((_n_steps-1)*state_dim,state_dim));
    errs(2*_n_steps-1) = time_min_constraint(std::abs(x(duration_start+_n_steps-2)));
    std::cout << "abolution value of x(duration_start+_n_steps-2): " << std::abs(x(duration_start+_n_steps-2)) << std::endl;
