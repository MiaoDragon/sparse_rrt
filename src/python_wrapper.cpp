...
public:
    ...
    ~DeepSMPWrapper()
    {
        neural_smp.reset();
        planner.reset();
        std::cout << "removed smp module" << std::endl;
    }
                    ...
                    double goal_radius, int max_iteration, py::object distance_computer_py, double delta_near, double delta_drain, double cost_threshold)
    {
        ...
        std::vector<double> res_t;
        //std::cout << "neural_smp planning" << std::endl;
        neural_smp->plan_tree_SMP(planner.get(), system, psopt_system, obs_tensor, start_state, goal_state, goal_inform_state, max_iteration, goal_radius, cost_threshold,
                         res_x, res_u, res_t);
        if (res_x.size() == 0)
        {
            std::cout << "python wrapper: solution length 0" << std::endl;
                    ...
                    int flag, double goal_radius, int max_iteration, py::object distance_computer_py, double delta_near, double delta_drain, double cost_threshold)
    {
        ...
        //std::cout << "neural_smp planning" << std::endl;
        neural_smp->plan_tree_SMP_step(planner.get(), system, psopt_system, obs_tensor, start_state, goal_state, goal_inform_state,
    ...
    py::object plan_tree_SMP_cost_step(std::string& planner_name, system_t* system, psopt_system_t* psopt_system, py::safe_array<double>& obs_py, py::safe_array<double>& start_py, py::safe_array<double>& goal_py, py::safe_array<double>& goal_inform_py,
                    int flag, double goal_radius, int max_iteration, py::object distance_computer_py, double delta_near, double delta_drain, double cost_threshold)
    {
        ...
        std::cout << "res_u.size: " << res_u.size() << std::endl;
         if (res_u.size() == 0)
         {
             ...
             std::cout << "mpnet_cost_res: " << std::endl;
             std::cout << mpnet_cost_res << std::endl;
             //delete planner;
             // return flag, available flags, states, controls, time
   ...
   ;
    ...
    py::class_<DeepSMPWrapper>(m, "DeepSMPWrapper")
        ...
        .def("plan", &DeepSMPWrapper::plan,
          ...
          .def("plan_tree_SMP", &DeepSMPWrapper::plan_tree_SMP,
              "planner_name"_a,
              "system"_a,
              "psopt_system"_a,
              "obs"_a,
          ...
          .def("plan_tree_SMP_step", &DeepSMPWrapper::plan_tree_SMP_step,
                "planner_name"_a,
                "system"_a,
                "psopt_system"_a,
                "obs"_a,
          ...
          .def("plan_tree_SMP_cost_step", &DeepSMPWrapper::plan_tree_SMP_cost_step,
                "planner_name"_a,
                "system"_a,
                "psopt_system"_a,
                "obs"_a,
