...
public:
	...
	virtual void step_bvp(system_interface* propagate_system, psopt_system_t* bvp_system, psopt_result_t& res, double* start_state, double* goal_state, int num_iters, int num_steps, double step_sz,
		const std::vector<std::vector<double>> &x_init,
		const std::vector<std::vector<double>> &u_init,
		const std::vector<double> &t_init) = 0;
    /**
